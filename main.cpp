        ...
        for(const Transition& ts : ov_){
            ...
            if(!ts.post_heads.empty()){
                ...
                for(const size_t h : ts.post_heads)
                    fmt::print(out,IND2 "l.FSM_HEAD_POST(HEAD_MAP[{}],pos_head);\n",h);
...
struct accumulate_ret_t{
    AC_Type type;
    unsigned height_i; ///< The index of the first empty or uncalculated intersections. Note that index `i` represents `i+2`-subsets.
    unsigned height_u; ///< Ditto.
};
...
accumulate_ret_t accumulate(const FSM_Edge_View* const edge_views,const unsigned total_edges){
    ...
    #define YUKI_LEX_ACCUMULATE_MAX_INTERSECTIONS 4096
    #endif
    #ifndef YUKI_LEX_ACCUMULATE_MAX_UNIONS
    #define YUKI_LEX_ACCUMULATE_MAX_UNIONS 4096
    #endif
    #ifndef YUKI_LEX_ACCUMULATE_FORCE_IU_LOWER_THRESHHOLD
    #define YUKI_LEX_ACCUMULATE_FORCE_IU_LOWER_THRESHHOLD 12
    #endif
    ...
    struct Acc_Impl{
        ...
        accumulate_ret_t accumulate_intersections_until_max(unsigned hi,const unsigned hu,size_t& total_intersections){
            assert(hu<hi); // So as to meet precondition 1 of `policy_edge_u`.
            for(;hi<total_edges_acc_impl-2;++hi){
                if(total_intersections>YUKI_LEX_ACCUMULATE_MAX_INTERSECTIONS)
                    return {AC_Type::U,hi,hu};
                const size_t si = make_intersections(hi);
                if(si!=0)
                    total_intersections+=si;
                else
                    break;
            }
        ...
        accumulate_ret_t accumulate_unions_until_max(const unsigned hi,unsigned hu,size_t& total_unions){
            for(;hu<total_edges_acc_impl-2;++hu){
                if(total_unions>YUKI_LEX_ACCUMULATE_MAX_UNIONS)
                    return {AC_Type::I,hi,hu};
                const size_t su = make_unions(hu);
                if(su!=0)
                    total_unions+=su;
                else
                    break;
    ...
    size_t total_intersections=0, total_unions=0;
    ...
    for(unsigned i=0; stem>1; stem>>=1,++i){
        ...
        for(unsigned j=i+1; added!=0; added>>=1,++j){
            ...
            if(!cc_temp.empty())
                intersectionss[0].emplace_back(stem+added,std::move(cc_temp),base_states+suffix_index(total_edges,stem));
        }
    }
    ...
    if(intersectionss[0].empty()){
        ...
        if(!unionss[0].empty()){
            total_unions+=unionss[0].size();
            ++hu;
            for(;hu<total_edges-2;++hu){
                if(total_unions>YUKI_LEX_ACCUMULATE_MAX_UNIONS)
                    return {AC_Type::I,0,hu};
                const size_t su = acc_impl.make_unions(hu);
                if(su!=0)
                    total_unions+=su;
                else
                    break;
        ...
        return {AC_Type::IU,0,hu};
    }

    total_intersections+=intersectionss[0].size();

    acc_impl.make_unions0();
    if(unionss[0].empty())
        return acc_impl.accumulate_intersections_until_max(1,0,total_intersections);

    total_unions+=unionss[0].size();
    }

    // Phase 1
    const unsigned min0 = total_edges-2<YUKI_LEX_ACCUMULATE_FORCE_IU_LOWER_THRESHHOLD-2?total_edges-2:YUKI_LEX_ACCUMULATE_FORCE_IU_LOWER_THRESHHOLD-2;

    for(unsigned hi=1;hi<min0;++hi){
        const size_t si = acc_impl.make_intersections(hi);
        if(si==0)
            return acc_impl.accumulate_unions_until_max(hi,1,total_unions);
        total_intersections+=si;
    }

    for(unsigned hu=1;hu<min0;++hu){
        const size_t su = acc_impl.make_unions(hu);
        if(su==0)
            return acc_impl.accumulate_intersections_until_max(min0,hu,total_intersections);
        total_unions+=su;
    }
    ...
    for(;h<(total_edges-2)*YUKI_LEX_ACCUMULATE_FORCE_IU_UPPER_THRESHHOLD;++h){
        ...
        if(intersections.empty())
            return acc_impl.accumulate_unions_until_max(h,h,total_unions);
        ...
        if(unions.empty())
            return acc_impl.accumulate_intersections_until_max(h+1,h,total_intersections);
...
accumulate_ret_t accumulate_simple(const FSM_Edge_View* const edge_views,const unsigned total_edges){
    unsigned total_intersections=0, total_unions=0;
    ...
    for(unsigned h=1;h<total_edges-2;++h){
        yuki::Vector<IEntry>& intersections = intersectionss[h];
        assert(intersections.empty());
        intersections.reserve(intersectionss[h-1].size()*(total_edges-h-1)/(h+2));
        for(IEntry& ie : intersectionss[h-1]){
...
void policy_edge_iu(const size_t fsms_size,const unsigned height_i,const unsigned height_u){
    ...
    for(int h=0;h<=(int)total_edges-4-(int)height_i;++h)
        unionss[h].clear();
...
void policy_edge_i(const size_t fsms_size,const unsigned height_i,const unsigned height_u){
        ...
        if(height_u){
            ...
            if(i_probe==unionss[height_u-1].end()){
                for(;!probe_stack.empty();++h_probe){
                    ...
                    i_probe = unionss[h_probe].emplace(top.insertion_locus,top.edge_indices);
                }
    ...
    for(unsigned i=0;i<total_edges;++i,stem>>=1){
        ...
        if(height_u){
            // At this point, `h_probe` should be `height_u`.
            if(find(unionss[height_u-1],n_probe)==unionss[height_u-1].end()){
                for(;!probe_stack.empty();++h_probe){
                    const auto top = probe_stack.pop();
                    unionss[h_probe].emplace_back(top.edge_indices);
                }
                goto probe_done;
            }
        }else{
...
void write(
    ...
    #ifdef YUKI_LEX_META_DBG
    ,const bool force_policy_char = false
    ,const bool acc_simple = false
    #endif
){
    ...
    #ifndef YUKI_LEX_META_EDGES_RESERVE
    #define YUKI_LEX_META_EDGES_RESERVE {4,4}
    #endif
    static Meta_Edges_Pair meta_edges_by_char[6] = {YUKI_LEX_META_EDGES_RESERVE,YUKI_LEX_META_EDGES_RESERVE,YUKI_LEX_META_EDGES_RESERVE,YUKI_LEX_META_EDGES_RESERVE,YUKI_LEX_META_EDGES_RESERVE,YUKI_LEX_META_EDGES_RESERVE};
    ...
    while(!worklist.empty()){
        ...
        if(is_headed.front())
            fprintf(out,IND "pos_head=l.in.get_pos();\n");

        for(const size_t head : current_state.post_heads_imm())
            fprintf(out,IND "l.FSM_HEAD_POST(HEAD_MAP[%zu],pos_head);\n",head);

        const FSM_Node* const current_accept = current_state.accept();
        if(current_accept){
            if(current_accept->is_tail)
                fprintf(out,IND "pos_take=l.FSM_TAIL(%zu,HEAD_MAP[%zu]);\n",current_accept->branch,current_accept->branch);
            else
                fprintf(out,IND "pos_take=l.FSM_TAKE(%zu);\n",current_accept->branch);
        }
        ...
        if(meta_actives!=0){
            ...
            #ifndef YUKI_LEX_META_REMOVE_RESERVE
            #define YUKI_LEX_META_REMOVE_RESERVE 16
            #endif
            static yuki::Vector<size_t> remove_list(yuki::reserve_tag,YUKI_LEX_META_REMOVE_RESERVE);
            ...
            switch(meta_actives){
                case 1:{
                    ...
                    switch(to_true_index[0]+1){
                        case MetaChar::BOL: fprintf(out,"l.FSM_META_BOL()");break;
                        case MetaChar::EOL: fprintf(out,"l.FSM_META_EOL()");break;
                        case MetaChar::BOI: fprintf(out,"l.FSM_META_BOI()");break;
                        case MetaChar::EOI: fprintf(out,"l.FSM_META_EOI()");break;
                        case MetaChar::EoF: fprintf(out,"l.FSM_META_EoF()");break;
                        case MetaChar::WB: fprintf(out,"l.FSM_META_WB()");break;
                    }
                    fprintf(out,")");
                    ...
                    if(!state0.post_heads.empty()){
                        ...
                        for(const size_t h : state0.post_heads)
                            fprintf(out,IND2 "l.FSM_HEAD_POST(HEAD_MAP[%zu],pos_head);\n",h);
                ...
                default:{
                    fprintf(out,IND "switch(l.make_meta_flags({");
                    for(unsigned i=meta_actives;i>1;){
                    ...
                    switch(to_true_index[0]+1){
                        case MetaChar::BOL: fprintf(out,"l.FSM_META_BOL()})){\n");break;
                        case MetaChar::EOL: fprintf(out,"l.FSM_META_EOL()})){\n");break;
                        case MetaChar::BOI: fprintf(out,"l.FSM_META_BOI()})){\n");break;
                        case MetaChar::EOI: fprintf(out,"l.FSM_META_EOI()})){\n");break;
                        case MetaChar::EoF: fprintf(out,"l.FSM_META_EoF()})){\n");break;
                        case MetaChar::WB: fprintf(out,"l.FSM_META_WB()})){\n");break;
                    }
                    ...
                    for(unsigned flags=0;flags<flags_end;++flags){
                        ...
                        if(!next.post_heads.empty()){
                            ...
                            for(const size_t h : next.post_heads)
                                fprintf(out,IND2 IND "l.FSM_HEAD_POST(HEAD_MAP[%zu],pos_head);\n",h);
                            next.post_heads.clear();
        ...
        switch(edge_views.size()){
            ...
            default:{
                #ifdef YUKI_LEX_META_DBG
                if(edge_views.size()==3 && !force_policy_char){
                ...
                #else
                if(force_policy_char || cc_union.char_count()<=YUKI_LEX_POLICY_CHAR_MAX_CHAR_COUNT || edge_views.size()>64 || !policy_edge(fsms_size,acc_simple))
                #endif
        ...
        fprintf(out,IND "return l.FSM_HALT(pos_take);\n");
...
int main(int argc,char** argv){
    ...
    if(argc>1){
        ...
        do{
            rp.parse();
        }while(!rl.matcher().at_end());
        write(fopen("out.txt","w"),rp.fsms.begin(),rp.fsms.size(),rp.max_branch_size,rp.max_branch_number);
        rewind(f);
        Regex_Lexer rl2(f);
        rp.lexer=&rl2;
        rp.recycle();
        do{
            rp.parse();
        }while(!rl2.matcher().at_end());
        write(fopen("out2.txt","w"),rp.fsms.begin(),rp.fsms.size(),rp.max_branch_size,rp.max_branch_number);
        #ifdef YUKI_LEX_META_DBG
        if(argc>2){
            bool fpc=false,as=false;
            for(char* p=argv[2];;++p){
                switch(*p){
                    case '\0': goto loop_outside;
                    case 'c': fpc=true;break;
                    case 's': as=true;break;
                }
            }
            loop_outside:
            if(fpc)
                write(fopen("out_char.txt","w"),rp.fsms.begin(),rp.fsms.size(),rp.max_branch_size,rp.max_branch_number,true,false);
            if(as)
                write(fopen("out_as.txt","w"),rp.fsms.begin(),rp.fsms.size(),rp.max_branch_size,rp.max_branch_number,false,true);
        }
        #endif
    }
}
